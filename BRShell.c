...
int comandos_internos(struct command *cmd)
{
	...
	else if (strcmp(cmd->argv[0], "cd") == 0 || strcmp(cmd->argv[0], "cd\n") == 0)
	{
		...
		if (strstr(cmd->argv[1], "~/"))
		{
			...
			char *path = sub_string(cmd->argv[1], 1, strlen(cmd->argv[1]));
		...
		else
		{
			...
			if(strstr(cmd->argv[1], "\n"))
			{
				path = sub_string(cmd->argv[1], 0, strlen(cmd->argv[1]) - 1);
			}
			else{
				path = sub_string(cmd->argv[1], 0, strlen(cmd->argv[1]) );
			}
	...
	else if (strcmp(cmd->argv[0], "historico") == 0 || strcmp(cmd->argv[0], "historico\n") == 0)
	{
		...
		if (cmd->argv[1] != NULL && !em_branco(cmd->argv[1]))
		{
...
{
	int built = comandos_internos(cmd);
	if (built != -1)
	{
		if (built == 0)
			exit(EXIT_SUCCESS);
...
int executar_comandos(struct commands *cmds)
{
	...
	if (cmds->cmd_count == 1)
	{
		...
		pid = executar_comando(cmds->cmds[0]);
		wait(NULL);
	}
...
int main(void)
{
	...
	while (1)
	{
		...
		if (strlen(input) > 0 && !em_branco(input) && input[0] && (boleano == 0))
		{

			struct commands *commands = parse_comandos_com_pipes(input);
			...
			else if ((boleano == 0))
			{
				...
				if (strstr(commands->cmds[i]->name, "./"))
				{
					...
					while (fgets(buffer, bufferLength, filePointer))
					{
						...
						{
							batch_commands = parse_comandos_com_pipes(buffer);
				...
				else
				{
					...
					executar_comando(commands->cmds[0]);
				}
			}
		}
